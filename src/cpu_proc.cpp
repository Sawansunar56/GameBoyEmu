...
function void proc_rlca(cpu_context *ctx)
{
 u8 u        = ctx->regs.a;
 b8 c        = (u >> 7) & 1;
 u           = (u << 1) | c;
...
function void proc_rrca(cpu_context *ctx)
{
 u8 b = ctx->regs.a & 1;
 ctx->regs.a >>= 1;
 ctx->regs.a |= (b << 7);
...
function void proc_stop(cpu_context *ctx) { fprintf(stderr, "STOPPING!\n"); }
...
function void proc_scf(cpu_context *ctx) { cpu_set_flags(ctx, -1, 0, 0, 1); }

function void proc_ccf(cpu_context *ctx)
{
 cpu_set_flags(ctx, -1, 0, 0, CPU_FLAG_C ^ 1);
}

function void proc_halt(cpu_context *ctx) { ctx->halted = true; }

function void proc_rra(cpu_context *ctx)
{
 u8 carry = CPU_FLAG_C;
...
function void proc_ei(cpu_context *ctx) { ctx->enabling_ime = true; }
...
function bool check_cond(cpu_context *ctx)
{
 bool z = CPU_FLAG_Z;
 bool c = CPU_FLAG_C;
...
function void goto_addr(cpu_context *ctx, u16 addr, bool pushpc)
{
 if (check_cond(ctx))
 {
  if (pushpc)
...
function void proc_add(cpu_context *ctx)
{
 ...
 {
  z = 0;
  h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) + (ctx->fetched_data & 0xF) >=
      0x10;
...
glob std::array<IN_PROC, IN_FINAL> processors = [] {
 std::array<IN_PROC, IN_FINAL> arr{};

 arr[IN_NONE] = proc_none;
 arr[IN_NOP]  = proc_nop;
 arr[IN_LD]   = proc_ld;
 arr[IN_LDH]  = proc_ldh;
 arr[IN_JP]   = proc_jp;
 arr[IN_DI]   = proc_di;
 arr[IN_POP]  = proc_pop;
 arr[IN_PUSH] = proc_push;
 arr[IN_JR]   = proc_jr;
 arr[IN_CALL] = proc_call;
 arr[IN_RET]  = proc_ret;
 arr[IN_RST]  = proc_rst;
 arr[IN_DEC]  = proc_dec;
 arr[IN_INC]  = proc_inc;
 ...
 arr[IN_CP]   = proc_cp;
 arr[IN_CB]   = proc_cb;
 arr[IN_RRCA] = proc_rrca;
 arr[IN_RLCA] = proc_rlca;
 arr[IN_RRA]  = proc_rra;
 arr[IN_RLA]  = proc_rla;
 arr[IN_STOP] = proc_stop;
 arr[IN_HALT] = proc_halt;
 arr[IN_DAA]  = proc_daa;
 arr[IN_CPL]  = proc_cpl;
 arr[IN_SCF]  = proc_scf;
 arr[IN_CCF]  = proc_ccf;
 arr[IN_EI]   = proc_ei;
 arr[IN_RETI] = proc_reti;
