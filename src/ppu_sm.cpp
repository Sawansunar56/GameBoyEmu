...
function void load_line_sprites()
{
 int cur_y = lcd_get_context()->ly;

 u8 sprite_height = LCDC_OBJ_HEIGHT;
 memset(ppu_get_context()->line_entry_array,
        0,
...
glob u32 target_frame_time = 1000 / 60;
glob s32 prev_frame_time   = 0;
glob s32 start_timer       = 0;
glob s32 frame_count       = 0;
  ...
  if (lcd_get_context()->ly >= YRES)
  {
   ...
   if (frame_time < target_frame_time)
   {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(target_frame_time - frame_time));
   }
