...
{
 fifo_entry *next = static_cast<fifo_entry *>(malloc(sizeof(fifo_entry)));
 next->next       = NULL;
 next->value      = value;
...
u32 fetch_sprite_pixels(int bit, u32 color, u8 bg_color)
{
 for (int i = 0; i < ppu_get_context()->fetched_entry_count; i++)
 {
  ...
  u8 lo = !!(ppu_get_context()->pfc.fetch_entry_data[(i * 2) + 1] & (1 << bit))
          << 1;
...
function bool pipeline_fifo_add()
{
...
void pipeline_load_sprite_tile()
{
 ...
 while (le)
 {
       ...
       (sp_x + 8) < ppu_get_context()->pfc.fetch_x + 8))
  {
   // need to add entry
   ppu_get_context()
       ->fetched_entries[ppu_get_context()->fetched_entry_count++] = le->entry;
  }
